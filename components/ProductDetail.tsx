...
import {
    Tag, Box, HelpCircle, CheckCircle2, AlertCircle, Play, Loader2, ArrowLeft,
    Search, Globe, ExternalLink, Store, ShoppingCart, MoreHorizontal,
    Database, Ruler, Scale, Palette, Globe2, FileText, Check, AlertTriangle, Download,
...
const statusConfig: Record<string, { color: string; label: string }> = {
    ...
    enriching: { color: "bg-indigo-500/10 text-indigo-400 border-indigo-500/20", label: "Processing..." },
    done: { color: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20", label: "Complete" },
...
export function ProductDetail({ productId }: ProductDetailProps) {
    ...
    // Auto-refresh when product is in a processing state
    useEffect(() => {
        if (!product) return;
        const activeStatuses = ['classifying', 'searching', 'extracting', 'validating', 'enriching'];
    ...
    const loadProduct = async () => {
        try {
            ...
            setProduct(data);
            // Reset button loading states when product loads
            ...
            if (!['classifying', 'searching', 'extracting', 'validating', 'enriching'].includes(data.status)) {
                setEnriching(false);
            }
        } catch (error) {
    ...
    // Use normalized data if available, else use extracted
    const displayData = validation ? validation.normalized_data : extraction;

    const isProcessing = ['classifying', 'searching', 'extracting', 'validating', 'enriching'].includes(product.status);
                    ...
                    <Card className={`border-zinc-800 backdrop-blur-sm ${classification ? 'bg-zinc-900/50' : 'bg-transparent border-dashed'}`}>
                        <CardHeader className="flex flex-row items-center justify-between pb-2 border-b border-zinc-800/50">
                            <div className="space-y-1">
                                <CardTitle className="text-sm uppercase tracking-widest text-zinc-400 flex items-center gap-2"><Box className="w-4 h-4 text-purple-400" /> Classification</CardTitle>
                            </div>
                        ...
                        {displayData ? (
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    {displayData.image_url?.value && (
                                        <TableRow className="border-zinc-800/50 hover:bg-zinc-800/30">
                                            <TableCell className="font-medium text-zinc-400 flex items-center gap-2" colSpan={4}>
                                                <div className="w-full p-2 relative group">
                                                    <img
                                                        src={`/api/image-proxy?url=${encodeURIComponent(displayData.image_url.value)}`}
                                                        alt="Product"
                                                        className="max-h-32 rounded border border-zinc-700 object-contain cursor-pointer hover:border-blue-500 transition-colors"
